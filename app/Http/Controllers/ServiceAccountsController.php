...
class ServiceAccountsController extends AppBaseController
{
    ...
    public function show($id)
    {
        ...
        $bills = DB::table('Billing_Bills')
            ...
            ->select('Billing_Bills.*',
                // DB::raw("(SELECT TOP 1 TRY_CAST(id AS VARCHAR) FROM Cashier_PaidBills WHERE AccountNumber=Billing_Bills.AccountNumber AND Cashier_PaidBills.ServicePeriod=Billing_Bills.ServicePeriod AND (Status IS NULL OR Status='Application') AND AccountNumber IS NOT NULL) AS ORNumber"),
    ...
    public function mwAdvanceMaterialDepositBalances(Request $request) {
        return view('/service_accounts/mw_adv_material_deposit_balances', [

        ]);
    }

    public function validateMwAdvanceMaterialDepositBalances(Request $request) {
        $file = $request->file('file');
        ...
        if ($file->isValid()) {
            ...
            if ($handle) {
                ...
                while (($line = fgets($handle)) !== false) {
                    ...
                    $zone = substr($line, 10, 3);
                    $block = substr($line, 14, 3);
                    $name = substr($line, 18, 28);
                    $acctNo = substr($line, 49, 8);
                    $meterNo = trim(substr($line, 58, 12));
                    $outBalance = str_replace(",", "", trim(substr($line, 76, 15)));
        ...
        Flash::success('Advanced material deposit balances updated!');
