        ...
        foreach ($rows as $key => $row) {
            if ($key > 1) { // skip header rows
                ...
                Bills::create([
                    ...
                    'BillingDate' => $row[12] != null ? (var_dump(Bills::validate(date('Y-m-d', strtotime(htmlspecialchars_decode(trim($row[12])))))) ? date('Y-m-d', strtotime(htmlspecialchars_decode(trim($row[12])))) : '1970-01-01' ) : null,
                    'ServiceDateFrom' => var_dump(Bills::validate(date('Y-m-15', strtotime($this->period . ' -1 month')))) ? date('Y-m-15', strtotime($this->period . ' -1 month')) : '1970-01-01'  ,
                    'ServiceDateTo' => var_dump(Bills::validate( date('Y-m-15', strtotime($this->period)))) ?  date('Y-m-15', strtotime($this->period)) : '1970-01-01' ,
                    'DueDate' => $row[13] != null ? (var_dump(Bills::validate( date('Y-m-d', strtotime(htmlspecialchars_decode(trim($row[13])))))) ?  date('Y-m-d', strtotime(htmlspecialchars_decode(trim($row[13])))) : '1970-01-01')  : null,
