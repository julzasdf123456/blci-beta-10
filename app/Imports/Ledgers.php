        ...
        foreach ($rows as $key => $row) {
            if ($key > 1) { // skip header rows
                ...
                Bills::create([
                    ...
                    'BillingDate' => $row[12] != null ? date('Y-m-d', strtotime(htmlspecialchars_decode(trim($row[12])))) : null,
                    'ServiceDateFrom' => date('Y-m-15', strtotime($this->period . ' -1 month')),
                    'ServiceDateTo' => date('Y-m-15', strtotime($this->period)),
                    'DueDate' => $row[13] != null ? date('Y-m-d', strtotime(htmlspecialchars_decode(trim($row[13])))) : null,
